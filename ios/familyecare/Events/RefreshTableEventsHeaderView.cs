...
{
	public class RefreshTableEventsHeaderView : UIView
	{
		...
		public RefreshTableEventsHeaderView () : base()
		{
			...
			if (NSUserDefaults.StandardUserDefaults["FEM_RefreshTableView_LastRefresh"] != null) {
				lastUpdatedLabel.Text = NSUserDefaults.StandardUserDefaults["FEM_RefreshTableView_LastRefresh"].ToString ();
			} else {
				SetCurrentDate ();
			}
			...
			SetStatus (RefreshTableEventsHeaderView.RefreshStatus.PullToReloadStatus);
			this.AddSubview (statusLabel);
			
			arrowImage = new UIImageView (new RectangleF (25f, this.Frame.Height - 65f, 30f, 55f));
			arrowImage.Image = UIImage.FromFile ("images/blackArrow.png");
		...
		public void SetStatus (RefreshStatus status)
		{
			...
			case RefreshStatus.LoadingStatus:
				this.statusLabel.Text = "Actualizando...";
				break;
				
			case RefreshStatus.PullToReloadStatus:
				this.statusLabel.Text = "Tira para actualizar...";
				break;
			case RefreshStatus.ReleaseToReloadStatus:
				this.statusLabel.Text = "Suelta para actualizar...";
				break;
			}
		}
		...
		{
			lastUpdatedLabel.Text = String.Format ("Última actualización: {0}", DateTime.Now.ToString ("G"));
			NSUserDefaults.StandardUserDefaults["FEM_RefreshTableView_LastRefresh"] = new NSString (DateTime.Now.ToString ("G"));
		...
		public void FlipImageAnimated (bool animated)
		{
			...
			arrowImage.Layer.Transform = isFlipped ? CATransform3D.MakeRotation (3.141593f * 2, 0f, 0f, 1f) : CATransform3D.MakeRotation (3.141593f, 0f, 0f, 1f);
